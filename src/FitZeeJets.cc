...
void myFit() {
  ...
  for(int nj = opts.getRealVal("njetmin"); nj <=opts.getRealVal("njetmax"); nj++){
    ...
    // signal mll 
    sprintf(speclabel,"Z%dj",nj);
    sprintf(pdfname,"sig_Mass%dj",nj);   
    theFit.addPdfWName("myFit", speclabel , "mee",  "Cruijff", pdfname);
...
void FitZElectrons() {
  ...
  if(opts.getBoolVal("AllFit")) {
    ...
    sprintf(initconfigfile,"fitconfig/fitZ-New-%s-thr%d.config",jetflavour,ithr);
