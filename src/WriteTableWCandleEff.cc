...
void writeTable(const char *dir, const char *ana = "Id") {
  ...
  for(int iwp=0; iwp<(int)WPs.size(); ++iwp) {
    ...
    for(int icut=0; icut<(int)cuts.size(); ++ icut) {
      ...
      RooFitResult *fitres = (RooFitResult*)file->Get("fitresult_myFit_T1");
      ...
      cout.precision(3);
      cout << idCuts[icut] << " & " 
           << "\t" << N_sig->getVal() << " $\\pm$ " << N_sig->getError() << " & "
           << "\t" << eff_sig->getVal() << " $^{+" << eff_sig->getAsymErrorHi() << "}_{" << eff_sig->getAsymErrorLo() << "}$ & " 
           << "\t" << N_qcd->getVal() << " $\\pm$ " << N_qcd->getError() << " & " 
           << "\t" << eff_qcd->getVal() << " $\\pm$ " << eff_qcd->getError()
           << " \\\\" << endl;
