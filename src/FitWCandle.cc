...
//     sprintf(treename,"theData");
  ...
  // Initialize the fit...
  if(opts.getBoolVal("AllFit")) theFit.initialize("fitconfig/WInclusive/fitWCandle.config");
  ...
  if(opts.getBoolVal("AllFit")) sprintf(configfilename, "fitres/WInclusive/fitMinimumWCandle-data.config");
...
void PlotWElectrons(int nbins) {
  ...
  theFit.addDataSetFromRootFile(treename, treename, datasetname);
  RooDataSet *data = theFit.getDataSet(treename);
  data = (RooDataSet*)data->reduce("nJets==0 && pfmet/pt>0.3");
  ...
  if(opts.getBoolVal("useMt")) {
    ...
    if(opts.getBoolVal("AllFit")) {
      sprintf(epsfilename,"fit-plots/eps/MtCandle-data.eps");
      sprintf(Cfilename,"fit-plots/macro/MtCandle-data.C");
    }
